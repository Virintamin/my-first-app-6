...
def posting(request):
    if request.method == "GET":
        ...
        if form.is_valid():
            #글쓴이를 저장하는 변수가 빈공간이므로 바로 데이터베이스에 저장하지
            #않고 Post객체로 변환
...
def searchP(request):
    ...
    #type : '0' 제목 검색
    if type=='0':
        #모델클래스.objects.filter : 특정조건을 만족하는 모듣객체
        #filter,get,exclude에 조건을 넣을때 (모델클래스의면수__명령어=값)
        #형태로 넣음
        #contains : 우변값이 해당 면수에 포함되있는 객체를 모두 추출
        list = Post.objects.filter(headline__contains=q)
        return render(request, "Blog/templates/searchP.html",
                      {'list':list} )
    elif type=='1':
